...
class AgentBehaviourNavigatorSwimmerNoisy : public sl0::sa0::Behaviour<AgentActiveStep> {
    public:
    ...
    public:
        AgentBehaviourNavigatorSwimmerNoisy(std::shared_ptr<Objects> sObjects) : sensorDirection(sObjects) {
        }
    public:
    ...
    public:
        void operator()(const double* pState, const double& t, const AgentActiveStep& stepActive) const override {
            ...
            TypeSpaceVector swimmingDirection = behaviourDirection(pState, t, stepActive, direction, velocityGradients).normalized();
