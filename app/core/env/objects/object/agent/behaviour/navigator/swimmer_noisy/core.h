#ifndef C0P_CORE_ENV_OBJECTS_OBJECT_AGENT_BEHAVIOUR_NAVIGATOR_SWIMMER_NOISY_CORE_H
#define C0P_CORE_ENV_OBJECTS_OBJECT_AGENT_BEHAVIOUR_NAVIGATOR_SWIMMER_NOISY_CORE_H
#pragma once
...
class AgentBehaviourNavigatorSwimmerNoisy : public sl0::sa0::Behaviour<typename AgentActiveStep::TypeStateStatic, TypeRef, AgentActiveStep> {
    public:
    ...
    public:
        AgentBehaviourNavigatorSwimmerNoisy(Objects<ObjectsParameters>* pObjects) {
        }
    public:
    ...
    public:
        void operator()(const TypeRef<const TypeAgentStateStatic>& state, const double& t, const AgentActiveStep& stepActive) const override {
            ...
            TypeSpaceVector swimmingDirection = behaviourDirection(state, t, stepActive, direction, velocityGradients).normalized();
            for(unsigned int i = 0; i < direction.size(); i++) {
                swimmingDirection[i] += parameters.noiseIntensity * rand0m::standardNormalDistribution(rand0m::engine);
            }
            swimmingDirection = swimmingDirection.normalized();
            // set velocity
