...
class AgentBehaviourTrackerLocalAxisSwimmer : public sl0::sa0::Behaviour<AgentActiveStep> {
	public:
	...
	public:
		AgentBehaviourTrackerLocalAxisSwimmer(std::shared_ptr<Objects> sObjects) : sensorPosition(sObjects) {
		}
	public:
	...
	public:
		void operator()(const double* pState, const double& t, const AgentActiveStep& stepActive) const override {
			...
			stepActive.sStepAxisOrient->direction = behaviourDirection(pState, t, stepActive, position, velocityGradients).normalized();
			stepActive.sStepLocalAxisSwim->velocity = behaviourVelocity(pState, t, stepActive, position, velocityGradients, swimmingDirection) * stepActive.parameters.velocity;
		}
};
