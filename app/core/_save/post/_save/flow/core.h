#ifndef C0P_CORE_POST_FLOW_CORE_H
#define C0P_CORE_POST_FLOW_CORE_H
#pragma once
...
class PostFlow {
    ...
    public:
        PostFlow(std::shared_ptr<Flow> p_sFlow, std::shared_ptr<PostMeshMesh::Type> p_sMesh) : sFlow(p_sFlow), sMesh(p_sMesh), parameters(sFlow, sMesh) {
        }
    public:
        std::map<std::string, TypeScalar> operator()(const double& t) {
            ...
            std::for_each(/*std::execution::par_unseq, */dataIndexs.begin(), dataIndexs.end(), [this, t, &processedData](const unsigned int& dataIndex){ 
                for(const auto& p : (*parameters.data[dataIndex])(t)) {
