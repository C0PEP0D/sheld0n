...
def edit(source, name, source_object_name = ""):
    ...
    find_replace(name, "*.h", "param/post/objects/{object_name}/{name}".format(name=source, object_name=source_object_name), "param/post/objects/{object_name}/{name}".format(name=name, object_name=object_name))
    find_replace(name, "*.h", "Post{object_name}{name}".format(name=upper_camel_source, object_name=upper_camel_source_object_name), "Post{object_name}{name}".format(name=upper_camel_name, object_name=upper_camel_object_name))
    # add object to param/post/parameters.h
    for line in fileinput.FileInput("parameters.h", inplace=True):
        if line == '// FLAG: INCLUDE POST END\n':
            print('#include "param/post/objects/{object_name}/{name}/choice.h"\n'.format(name=name, object_name=object_name), end='')
        if line == '        // FLAG: MAKE POST END\n':
            print('        data.push_back(std::make_shared<Post{object_name}{name}<Type{object_name}Step>>(s{object_name}Step));\n'.format(name=upper_camel_name, object_name=upper_camel_object_name), end='')
        print(line, end='')
