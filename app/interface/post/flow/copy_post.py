...
def edit(source, name):
    ...
    find_replace(name, "*.h", "param/post/flow/{name}".format(name=source), "param/post/flow/{name}".format(name=name))
    find_replace(name, "*.h", "PostFlow{name}".format(name=source.capitalize()), "PostFlow{name}".format(name=name.capitalize()))
    # add post to param/post/flow/parameters.h
    for line in fileinput.FileInput("parameters.h", inplace=True):
        if line == '// FLAG: INCLUDE POST END\n':
            print('#include "param/post/flow/{name}/choice.h"\n'.format(name=name), end='')
        if line == '        // FLAG: MAKE POST END\n':
            print('        data.push_back(std::make_shared<PostFlow{name}>(sFlow, sMesh));\n'.format(name=name.capitalize()), end='')
        print(line, end='')
