...
def edit(source, name):
    ...
    for line in fileinput.FileInput("../parameters.h", inplace=True):
        if line == '// FLAG: INCLUDE OBJECT END\n':
            print('#include "param/env/objects/manager/{}/choice.h"\n'.format(name), end='')
        ...
        if line == '\t\t// FLAG: MAKE OBJECT END\n':
            ...
            print('\t\tsObjectsManagerSteps.push_back(s{Name}Step);\n'.format(Name=upper_camel_name), end='')
            print('\t\t{name}Index = sObjectsManagerSteps.size() - 1;\n'.format(name=name), end='')
            print('\t\tobjectsManagerNames.push_back("{name}");\n'.format(name=name), end='')
        print(line, end='')
    # post
    ## replace source by name
    ...
    for line in fileinput.FileInput("../../../init/objects/parameters.h", inplace=True):
        ...
        if line == '\t\t// FLAG: MAKE OBJECT END\n':
            print('\t\tsInitsManager.push_back(std::make_shared<InitInitManager<Init{Name}Parameters, {Name}Step>>(sObjectsParameters->s{Name}Step));\n'.format(Name=upper_camel_name), end='')
        print(line, end='')
