...
def edit(name):
    ...
    for line in fileinput.FileInput("../parameters.h", inplace=True):
        if (line == '#include "param/env/objects/manager/{}/choice.h"\n'.format(name)):
            pass
            ...
            pass
        elif (line == '\t\tsObjectsManagerSteps.push_back(s{Name}Step);\n'.format(Name=upper_camel_name)):
            pass
        elif (line == '\t\t{name}Index = sObjectsManagerSteps.size() - 1;\n'.format(name=name)):
            pass
        elif (line == '\t\tobjectsManagerNames.push_back("{name}");\n'.format(name=name)):
            pass
        else:
            print(line, end='')
    ...
    for line in fileinput.FileInput("../../../init/objects/parameters.h", inplace=True):
        if (line == '#include "param/init/objects/{}/parameters.h"\n'.format(name)):
            pass
        elif (line == '\t\tsInitsManager.push_back(std::make_shared<InitInitManager<Init{Name}Parameters, {Name}Step>>(sObjectsParameters->s{Name}Step));\n'.format(Name=upper_camel_name)):
            pass
        else:
            print(line, end='')
