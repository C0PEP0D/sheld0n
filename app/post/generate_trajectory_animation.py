...
def parse():
    ...
    parser.add_argument('--number', '-n', type=int, default=0, help='number of particles')
    return parser.parse_args()

def main(input_equation_list, input_color_list, input_step, number):
    # equations
    ...
    for time_index, time in enumerate(time_array):
        if time_index % input_step == 0:
            ...
            for equation_index, equation_name in enumerate(equation_name_list):
                if number > 0:
                    arts = art_ax.plot(pos_0_over_time[equation_name][0:time_index+1:input_step, :number], pos_1_over_time[equation_name][0:time_index+1:input_step, :number], color=color_list[equation_index % len(color_list)])
                    artists[-1] += arts
                    art = art_ax.scatter(pos_0_over_time[equation_name][time_index, :number], pos_1_over_time[equation_name][time_index, :number], color=color_list[equation_index % len(color_list)])
...
if __name__ == '__main__':
    args = parse()
    main(args.equation_list, args.color_list, args.step, args.number)
