...
struct LearnParameters {
	...
	struct RLEnvParameters {
		...
		static constexpr unsigned int SimulationStepNumberPerLearningStep = 16;
		static constexpr double Dt = SimulationStepNumberPerLearningStep * RunParameters::Dt;
		static constexpr unsigned int EpisodeStepLimit = RunParameters::NTime/SimulationStepNumberPerLearningStep; // number of step before simulation ends
	...
	struct LoopCoreParameters: rlt::rl::algorithms::ppo::loop::core::DefaultParameters<tScalar, tIndex, tEnv>{
		...
		// critic
		static constexpr tIndex CRITIC_HIDDEN_DIM = 64;
		static constexpr auto CRITIC_ACTIVATION_FUNCTION = rlt::nn::activation_functions::ActivationFunction::RELU;

		// env parameters
		static constexpr tIndex N_ENVIRONMENTS = 8;//std::pow(2, 7);
		static constexpr tIndex ON_POLICY_RUNNER_STEPS_PER_ENV = 64;//std::pow(2, 13);
		static constexpr tIndex BATCH_SIZE = 64;//std::pow(2, 13);
		// static constexpr tIndex TOTAL_STEP_LIMIT = 128;//std::pow(2, 24);

		static constexpr tIndex STEP_LIMIT = 1024;//TOTAL_STEP_LIMIT/(ON_POLICY_RUNNER_STEPS_PER_ENV * N_ENVIRONMENTS) + 1;
	...
	struct tLoopEvalParameters: rlt::rl::loop::steps::evaluation::Parameters<tScalar, tIndex, tNext>{
	    static constexpr tIndex EVALUATION_INTERVAL = 1;
