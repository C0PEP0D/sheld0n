...
struct _PassiveParticlesParameters {
	...
	static std::map<std::string, tScalar> post(const double* pState, const double t) {
		...
		for(unsigned int subIndex = 0; subIndex < Number; ++subIndex) {
			...
			output["passive_particles__index" + std::format("{:0>{}d}", subIndex, FormatNumber) + "__dir_0"] = p[0];
			output["passive_particles__index" + std::format("{:0>{}d}", subIndex, FormatNumber) + "__dir_1"] = p[1];
			xAverage += x;
		}
		xAverage /= Number;
