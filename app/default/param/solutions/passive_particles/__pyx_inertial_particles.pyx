...
#   x: particle position
#   v: particle velocity
cdef public void passive_particles_constrain(const double t, c0p.tViewSpaceVector x, c0p.tViewSpaceVector v) noexcept nogil:
	pass
...
cdef public void passive_particles_init(const unsigned int particle_number, c0p.tViewSpaceVector* x_array, c0p.tViewSpaceVector* v_array) noexcept nogil:
...
# input:
#   x_array: array of particle position
#   v_array: array of particle position
#   particle_number: number of particle
#   t: current time
# output:
#   output_dictionary: dictionary that contains the post processed data
cdef public void passive_particles_post(c0p.tViewConstSpaceVector* x_array, c0p.tViewConstSpaceVector* v_array, const unsigned int particle_number, const double t, std.map[std.string, double]& output_dictionary) noexcept nogil:
	...
	cdef c0p.tSpaceVector v_average = c0p.tSpaceVector.Zero()
	...
	for particle_index in range(particle_number):
		...
		# velocity
		for dim_index in range(v_array[particle_index].size()):
			# zero padding
			key = std.string('passive_particles__index_')
			for key_index in range(format_particle_number - format_particle_index):
				key += std.string('0')
			...
			key += std.to_string(dim_index)
			# output
			output_dictionary[key] = v_array[particle_index][dim_index]

		# add position to average
		x_average += x_array[particle_index]
		v_average += v_array[particle_index]

	# end computation of average
	x_average /= particle_number
	v_average /= particle_number
	...
	for dim_index in range(v_average.size()):
		# zero padding
		key = std.string('passive_particles__average_v_')
		key += std.to_string(dim_index)
		output_dictionary[key] = v_average[dim_index]
