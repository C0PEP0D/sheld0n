...
struct _PassiveParticlesParameters {
	...
	static const unsigned StateSize = DIM + 2; // dimension of the state variable (position + angle + dof)
	...
	struct tMemberVariable : public d0t::VariableVector<tVector, tView, StateSize> {
		...
		static void constrain(std::vector<std::vector<double>>& stateArray, const double t, const unsigned int memberStateIndex) {
			...
			double* theta = pState + DIM;
			double* dof = pState + DIM + 1;
			// cython
			passive_particles_constrain(t, x, *theta, *dof);
	...
	struct tMemberEquation : public d0t::Equation<tMemberVariable> {
		...
		static void prepare(const double* pState, const unsigned int stateSize, const double t) {
			...
			const double theta = *(pState + DIM);
			const double dof = *(pState + DIM + 1);
			// cython
			passive_particles_prepare(x, theta, dof, t);
		...
		static tStateVectorDynamic stateTemporalDerivative(const double* const * pStateArray, const unsigned int* pStateSize, const unsigned int arraySize, const double t, const unsigned int memberIndex) {
			...
			double* dtheta = dState.data() + DIM;
			double* ddof = dState.data() + DIM + 1;

			// cython
			passive_particles_state_temporal_derivative(x, theta, dof, t, dx, *dtheta, *ddof);
			// ---------------- CUSTOM EQUATION END
	...
	static void init(double* pState) {
		...
		std::vector<double*> dofArray;
		xArray.reserve(Number);
		for(unsigned int index = 0; index < Number; ++index) {
			xArray.emplace_back(pState + index * StateSize);
			thetaArray.emplace_back(pState + index * StateSize + DIM);
			dofArray.emplace_back(pState + index * StateSize + DIM + 1);
		}

		// cython
		passive_particles_init(Number, xArray.data(), thetaArray.data(), dofArray.data());
	...
	static std::map<std::string, tScalar> post(const double* pState, const double t) {
		...
		// cython
		passive_particles_post(xArray.data(), thetaArray.data(), dofArray.data(), Number, t, output);
