...
#include "param/init/objects/mobject/parameters.h"
...
struct InitObjectsParameters {
	...
	InitObjectsParameters(std::shared_ptr<TypeObjectsParameters> sObjectsParameters) {
		...
		sInitsManager.push_back(std::make_shared<InitInitManager<InitMobjectParameters, MobjectStep>>(sObjectsParameters->sMobjectStep));
