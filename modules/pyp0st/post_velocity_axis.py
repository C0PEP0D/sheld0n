...
def main(axis):
    ...
    for object_name in fits_objects:
        ...
        for us, rt, sn, dn, jn in us_sorted:
            ...
            # value
            fit, cov = sp.optimize.curve_fit(fit_func, us_sorted[(us, rt, sn, dn, jn)][:, 3], us_sorted[(us, rt, sn, dn, jn)][:, 0])
            fits_objects[object_name]["value"] += [fit_func(fits_objects[object_name]["value"][0], fit[0], fit[1], fit[2], fit[3])]
            # max
