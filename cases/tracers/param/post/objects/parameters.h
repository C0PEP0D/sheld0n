...
#include "param/post/objects/tracer/parameters.h"
...
struct PostObjectsParameters {
	...
	PostObjectsParameters(std::shared_ptr<ObjectsParameters> sObjectsParameters) {
		...
		sPostsStatic.push_back(std::make_shared<PostPost<PostTracerParameters, TracerStep>>(sObjectsParameters->sTracerStep));
